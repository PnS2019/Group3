...
for i in range(num_epochs):
  for j in range(train_x // batch_size):
    input1 = train_X[j * batch_size : (j+1) * batch_size]
    target1 = train_y[j * batch_size : (j+1) * batch_size]
    train_function(input1,[target1,np.newaxis])
